...
export const HeroScrollVideo: React.FC<HeroScrollVideoProps> = ({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Target size - TRUE fullscreen immersive takeover
      const target = (() => {
  ...
  return (
    <div
      ...
      style={{ ...cssVars, ...style, width: '100vw', position: 'relative', left: '50%', marginLeft: '-50vw' }}
    >
      {/* Headline/hero area */}
      ...
      <style>{`
        ...
        .hsv-title {
          ...
          text-wrap: balance;
          background: linear-gradient(90deg, #e5e7eb 0%, #e5e7eb 50%, var(--accent) 100%);
